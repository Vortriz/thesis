...
// Base taken from https://github.com/coli-saar/bananote/blob/main/lib.typ. MIT License attached as LICENSE in the same directory.
...
#let note(
    ...
    for (i, author) in authors.enumerate() {
        ...
        let affiliation = if type(author) == array and author.len() > 1 {
            author.at(1)
        } else { "" }
    ...
    show heading.where(level: 1): it => {
        block(below: 1em, above: 2em)[ // block ensures that title and yellow box stay together on pagebreaks
            ...
            #text(font: sans, weight: sans-weight, size: 14pt, it.body)
        ]
        // v(-0.2em)
    }
    ...
    let maybe-highlight(reference) = {
        if type(highlight-by) == array {
            highlight-by.any(name => (
                name in family-names(reference.fields.labelname)
            ))
        } else {
    ...
    show link: it => if-citation(it, value => {
        let color = if maybe-highlight(value.reference) { darkgreen } else {
            darkblue
        }
        set text(fill: color)
        it
