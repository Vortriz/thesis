...
def plot_loss_evaluation_vs_initial(
    ...
    loss_callable = (
        mmd_distance
        if loss_function == LossFunction.MMD
...
class ForwardProcess:
    ...
    def _gen_initial_forward_states(self, rng):
        ...
        """
        all_zeros_basis_state = np.ones(
            (self.n_forward_samples, 1), dtype=np.complex128,
        )
        other_basis_states = rng.standard_normal(
            (self.n_forward_samples, 2**self.n_qubits - 1),
        ) + 1j * rng.standard_normal((self.n_forward_samples, 2**self.n_qubits - 1))

        states = np.hstack((all_zeros_basis_state, self.scale * other_basis_states))
...
class BackwardProcess(nn.Module):
    ...
    def _train_step(self, t, rng):
        ...
        denoising_loss : np.ndarray, shape (epochs,)
            The MMD/Wasserstein loss between the model's output and the true initial state, recorded for each epoch.
        """
        ...
        for epoch in progress(range(self.epochs), title="Epochs", remove_on_exit=True):
            ...
            output_t = self._get_denoised_state(
                self.forward_states[t + 1, indices], params,
            )
            training_loss = loss_callable(output_t, true_data)
            loss_wrt_target = loss_callable(
                output_t, self.forward_states[0, indices],
            )
            optimizer.zero_grad()
            loss_wrt_target.backward()
            # training_loss.backward()
            optimizer.step()
            denoising_loss[epoch] = loss_wrt_target.item()

            if epoch % 100 == 0:
                print(f"DEBUG: t={t}, Step {epoch:04d}, loss: {loss_wrt_target.item():.5f}")
