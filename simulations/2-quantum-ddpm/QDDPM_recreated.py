...
def _():
    fp1 = ForwardProcess(
        ...
        scale=0.08,
        T=20,
        schedule=np.linspace(0.3, 2.75, 20),
    )
    ...
    mo.hstack(
        [
            mo.vstack(
                [
                    slider,
                    mo.mpl.interactive(plot_bloch_sphere(fp1.forward_states[slider.value])).style(
                        width="510px",
                    ),
                ],
                align="center",
...
@app.cell
def _(fp1):
    bp1 = BackwardProcess().from_forward_process(fp1, n_ancilla=1, n_layers=4, n_backward_samples=100, epochs=2000, loss_function=LossFunction.WASSERSTEIN)
...
def _(bp1_loaded):
    plot_loss_training_vs_initial(
        ...
        title="Loss distance of Training Ensemble with respect to Initial Ensemble",
    )
    return
...
def _(qutip):
    def bloch_helper_alt(ax, sphere, samples):
        ...
        for i in range(samples.shape[0]):
            state = samples[i][0] * qutip.basis(2, 0) + samples[i][1] * qutip.basis(2, 1)
            points[i] = [
...
def _():
    fp2 = ForwardProcess(
        ...
        scale=0.02,
        T=10,
        schedule=np.linspace(0.3, 3.5, 10),
    )
...
def _(fp2):
    bp2 = BackwardProcess().from_forward_process(
        fp2, n_ancilla=2, n_layers=10, n_backward_samples=100, epochs=2000, loss_function=LossFunction.MMD
    )
...
@app.cell
def _():
    from QDDPM import wasserstein_distance
    return


@app.cell
def _(bp1_loaded, forward_states1):
    torch.abs(torch.from_numpy(forward_states1[0][:100]).conj() @ bp1_loaded.run(state=32)[0].T)
    return
