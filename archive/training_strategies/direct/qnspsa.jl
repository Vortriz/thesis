...
struct DirectQNSPSA <: DirectStrategy
    ...
    function DirectQNSPSA(;
        ...
        return new(loss_function, [n_iters], hyper_params, loss_history)
    end
end

function run_direct_process(
    model::Model,
    params_matrix::Matrix{Float64},
    initial_reg::ConcreteBatchedArrayReg;
    fix_outcomes=nothing,
)
...
function get_direct_state_overlap(
    model::Model,
    initial_reg::ConcreteBatchedArrayReg,
    params1::Matrix{Float64},
    params2::Matrix{Float64},
)
...
function get_direct_raw_metric_tensor(
    ...
    for i in 1:4
        push!(
            tasks,
            Threads.@spawn get_direct_state_overlap(
                model,
                input_reg,
                params,
                [p_pp, p_p, p_mp, p_m][i],
