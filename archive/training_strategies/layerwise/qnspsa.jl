...
struct QNSPSA <: SequentialStrategy
    ...
    function QNSPSA(;
        ...
        return new(loss_function, iter_schedule, hyper_params, loss_history)
    end
end

# Helper to get state before measurement for Metric Tensor calculation
function get_unitary_output(
    model::Model,
...
function get_state_overlap(
    model::Model,
    input_reg::ConcreteBatchedArrayReg,
    params1::Vector{Float64},
    params2::Vector{Float64},
)
...
function get_raw_metric_tensor(
    ...
    for i in 1:4
        push!(
            tasks,
            Threads.@spawn get_state_overlap(
                model,
                input_reg,
                params,
                [p_pp, p_p, p_mp, p_m][i],
...
function train_step!(
    ...
    @progress for k in 1:strategy.iter_schedule[t]
        ...
        d_plus_task = Threads.@spawn denoise(model, strategy, current_reg, p_plus)
        d_minus_task = Threads.@spawn denoise(model, strategy, current_reg, p_minus)

        d_plus, d_minus = fetch.([d_plus_task, d_minus_task])
        ...
        d_next_task = Threads.@spawn denoise(model, strategy, current_reg, params_next)

        d_curr, d_next = fetch.([d_curr_task, d_next_task])
