...
struct GradEnzyme{F, O} <: SequentialStrategy
    ...
    function GradEnzyme(; loss_function::F, optimizer::O, iter_schedule) where {F, O}
        ...
        return new{F, O}(loss_function, optimizer, iter_schedule, loss_history)
    end
end

@inline function apply_rx_enzyme!(
...
@inline function apply_ry_enzyme!(
    state::Matrix{ComplexF64},
    q::Int,
    θ::Float64,
    n_total::Int,
...
@inline function apply_backward_circuit_enzyme!(
    state::Matrix{ComplexF64},
...
function manual_measure_ancillas!(
    outcomes::Vector{Int},
    probs_buf::Vector{Float64},
    state::Matrix{ComplexF64},
    d::Int,
    a::Int,
    n_batch::Int,
...
function train_step!(
    ...
    strategy::GradEnzyme,
    t::Int,
    autoregressive_reg::ConcreteBatchedArrayReg,
)
